...
public final class Http1ExchangeCodec implements ExchangeCodec {
    ...
    @Override // okhttp3.internal.http.ExchangeCodec
    public void writeRequestHeaders(Request request) throws Throwable {
        Intrinsics.checkParameterIsNotNull(request, "request");
    ...
    public final void writeRequest(Headers headers, String requestLine) throws Throwable {
    ...
    public final class ChunkedSource extends AbstractSource {
        ...
        final /* synthetic */ Http1ExchangeCodec this0;
        ...
        public long read(Buffer sink, long byteCount) throws IOException {
            ...
            if (read == -1) {
                this.this0.getConnection().noNewExchanges();
                ProtocolException protocolException = new ProtocolException("unexpected end of stream");
                responseBodyComplete$okhttp();
                throw protocolException;
            }
        ...
        private final void readChunkSize() throws IOException {
            if (this.bytesRemainingInChunk != -1) {
                this.this0.source.readUtf8LineStrict();
            }
            try {
                this.bytesRemainingInChunk = this.this0.source.readHexadecimalUnsignedLong();
                String readUtf8LineStrict = this.this0.source.readUtf8LineStrict();
                if (readUtf8LineStrict == null) {
                    throw new TypeCastException("null cannot be cast to non-null type kotlin.CharSequence");
                }
                String obj = StringsKt.trim((CharSequence) readUtf8LineStrict).toString();
                if (this.bytesRemainingInChunk >= 0) {
                    if (!(obj.length() > 0) || StringsKt.startsWith(obj, ";", false)) {
                        if (this.bytesRemainingInChunk == 0) {
                            this.hasMoreChunks = false;
                            Http1ExchangeCodec http1ExchangeCodec = this.this0;
                            http1ExchangeCodec.trailers = http1ExchangeCodec.readHeaders();
                            OkHttpClient okHttpClient = this.this0.client;
                            if (okHttpClient == null) {
                                Intrinsics.throwNpe();
                            }
                            CookieJar cookieJar = okHttpClient.cookieJar();
                            HttpUrl httpUrl = this.url;
                            Headers headers = this.this0.trailers;
                            if (headers == null) {
                                Intrinsics.throwNpe();
                            }
