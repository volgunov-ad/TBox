...
public final class TaskRunner {
    ...
    public interface Backend {
        ...
        void coordinatorWait(TaskRunner taskRunner, long j) throws InterruptedException;
        ...
        this.runnable = new Runnable() { // from class: okhttp3.internal.concurrent.TaskRunner$runnable$1
            ...
            public void run() {
                ...
                while (true) {
                    ...
                    if (isLoggable) {
                        j = queue$okhttp.getTaskRunner().getBackend().nanoTime();
                    ...
                    try {
                        ...
                        if (isLoggable) {
                            long nanoTime = queue$okhttp.getTaskRunner().getBackend().nanoTime() - j;
                            TaskLoggerKt.access$log(awaitTaskToRun, queue$okhttp, "finished run in " + TaskLoggerKt.formatDuration(nanoTime));
                        }
                    } catch (Throwable th) {
                        if (isLoggable) {
                            long nanoTime2 = queue$okhttp.getTaskRunner().getBackend().nanoTime() - j;
    ...
    private final void afterRun(Task task, long j) {
        if (!Util.assertionsEnabled || Thread.holdsLock(this)) {
            ...
            if (j != -1 && !cancelActiveTask$okhttp && !queue$okhttp.getShutdown()) {
