...
| **turnHint** | Применённый hint поворотника: `L` / `R`. `-` если поворотник выкл., аварийка, в ranked нет связанного кандидата ≥25° в сторону стебля (перестроение / ранний `*_link` почти прямо), или текущее ребро — изогнутая односторонняя дуга (кольцо). Не отклеивает шайбу. |
