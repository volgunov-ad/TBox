...
| **matchMode** | `ORDINARY` (softCorrect), `RAILS` (коридор: граф выбирает ребро, поза следует free + поперечный снэп) или экспериментальный `FREE_TURNS` (Обычный + сильный курс + отвязка за 30 м до узла >3 линий до 10 м после). Default Ordinary. |
