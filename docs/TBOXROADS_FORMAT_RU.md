...
| `maxspeed` | int (опц.) | Числовой лимит км/ч. Нет поля — неизвестно. Не пишем `RU:urban`, `signals`, `walk`, `none`, `conditional` |
...
Tool режет OSM way только в **узлах-перекрёстках** (узел входит в ≥2 way). Точки излома одной дороги (`degree = 2`) не режутся. Старые пакеты без новых полей валидны: загрузчик пропускает неизвестные ключи, лимит = неизвестен. `format` остаётся `1`.
