...
| `maxspeed` | int (опц.) | Числовой лимит км/ч. Нет поля — неизвестно. `RU:urban`→60, `RU:rural`→90, `RU:motorway` и неразмеченный `highway=motorway` (не `motorway_link`)→110. Не пишем `signals`, `walk`, `none`, `conditional` |
| `maxspeedForward` / `maxspeedBackward` | int (опц.) | OSM `maxspeed:forward` / `maxspeed:backward` вдоль / против `coords` |
| `ref` | string (опц.) | Номер дороги (`M-7`, `А-108`). Без `name` |
...
(`allowAgainstOneway` при reverse gear). Если рядом (xt ≤ 40 м) есть дорога
«по ходу» класса motorway…tertiary — against-кандидаты **убираются из beam**
(не липнем к встречной полосе dual carriageway). Старые пакеты без поля = двусторонние.
...
- Новые поля ребра (`maxspeed`, `maxspeedForward`, `maxspeedBackward`, `ref`, `wayId`) опциональны. Старый загрузчик их пропускает (`skipValue`); матчер на обновлённых пакетах продолжает работать. Новый загрузчик на старых пакетах даёт `null` лимит.

### Lookahead лимита (виджет OSM)

`SpeedLimitLookahead` читает уже загруженные тайлы (`RoadGraphStore.cachedGraphs`), не пакет заново.

- Горизонт **500 м** вдоль графа от текущей позиции на matched-ребре.
- «Дальше» = первый **другой** числовой лимит на предполагаемом прямом пути.
- На развилке исходящие с курсом **> 65°** относительно азимута **конца текущего ребра** (не GNSS) отбрасываются — считаем, что едем прямо.
- Неизвестный `maxspeed` **не перескакиваем**: текущий числовой + следующее ребро без числа → «дальше» нет. Если текущий неизвестен, а впереди есть число — это «дальше».
- Два и более прямых исходящих в конусе 65°: если ветки не согласны по следующему лимиту (`null` ≠ число) — «дальше» скрыто (`nextLimitHidden`). Если согласны — берём это значение и **меньшую** дистанцию.
- T-образный стык, где оба варианта > 65° — lookahead обрывается (это нормально).
- Встречный `oneway` не берём, кроме заднего хода (`allowAgainstOneway`).
- Это **не** короткий match-коридор `advanceAlongTopology` (10–20 м / 8 hops).
- Полный обход графа **не** на каждом тике матчера: текущий лимит всегда O(1), дистанция «дальше» считается от последнего прохода. Повторный обход — смена ребра/направления, скачок along, вход в горизонт 500 м, истечение countdown, либо каждые **50 м / 4 с**.
