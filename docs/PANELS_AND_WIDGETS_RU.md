...
- `useMbCanVhal`, `httpRequestYaml`, поля поездки (`tripWidgetSource`, `tripMetricFieldId` для `tripMetricWidget`), `avgFuelConsumptionSource` (только `averageFuelConsumption`), `selectedDriveMode` (кнопка режима), `selectedDriveModes` (цикл режимов) и др.
...
Виджет **«Список программ»** (`appListWidget`, раздел «Система»): по нажатию открывает диалог со всеми launchable-приложениями (тот же источник, что у «Ярлык приложения»), по алфавиту, с иконкой и названием. Справа у каждой строки: **Открыть** (обычный fullscreen-запуск), **Скрыть** / **Показать**, **Удалить** (сначала подтверждение в TBox, затем системный `ACTION_DELETE` / `package:`; для чистых system-пакетов кнопка disabled). Перед системным UI диалог закрывается и при необходимости снимается window-mode overlay (как при fullscreen-запуске), иначе `TYPE_APPLICATION_OVERLAY` перекрывает установщик. В манифесте объявлен `REQUEST_DELETE_PACKAGES` (нужен с Android 9). Скрытые пакеты хранятся в настройках (`app_list_hidden_packages`) и действуют **только** в этом списке. Переключатель **«Показать скрытые»** внизу справа — только на время открытого диалога. Иконка плитки — `dashboard.system.app_list` (заменяется в «Настроить иконки…»). Заголовок плитки настраивается как у ярлыка (showTitle / customTitle; позиция по умолчанию снизу).
