...
Документ описывает три поверхности отображения **плиток** (виджетов) в TBox Monitor (**1.0.0**), общую модель данных и **как добавить новый виджет** в код.

Пользовательские шаги настройки — в [USER_GUIDE_RU.md](USER_GUIDE_RU.md) (главный экран, плавающие панели, вкладка «Плитки»). Права для дня/ночи и регулировки зеркал — ниже в этом документе. Экспорт панелей в темы — в [Themes.md](Themes.md).

Актуальное для 1.0.0: HVAC (вентилятор, температуры, обдув, SYNC), багажник, складывание/регулировка зеркал, тема день/ночь, **выбор страницы главного экрана** (`mainScreenPageSelectorWidgetHorizontal` / `…Vertical`: 1…N кнопок по `pageCount`; в оконном режиме пишет `currentPageWindowMode` и снимает session pin), SLA/знак ограничения скорости, **ограничения скорости по картам, камеры и радары** (`osmSpeedLimitWidget`: текущий лимит OSM / последний радар, камера впереди по пути, следующий лимит по пути или камере; колонки настраиваются), **круиз-контроль (ACC и обычный CCS)**, stepper (громкость и HVAC), настраиваемый вид плитки (выравнивание, вес шрифта, положение заголовка, зазор сетки, отступы содержимого), шаг сетки панелей, описания типов в диалоге выбора; давление/температура шин через TBox или mbCAN/VHAL; **загрузка CPU и свободная ОЗУ** головного устройства (`cpuUsage`, `freeRamPercent`); **газ/тормоз** (`gasBrakeWidget`: % педали газа с ГУ; красный текст при нажатом тормозе); **обслуживание дворников** (`wiperMaintenanceWidget`: тап — сервис On/Off; иконка Off / INT / Low / High по `WiperSts`); **средний расход топлива** (`averageFuelConsumption`: источник в «Дополнительно» — mbCAN/VHAL / текущая поездка / суточная поездка); **параметр поездки** (`tripMetricWidget`: один показатель текущей или суточной поездки).

Встроенные drawable-иконки виджетов можно заменить глобально через **Настройки → Настроить
иконки виджетов и меню**. Для динамических виджетов каждое состояние (например, режим обдува,
уровень сети или положение дворников) имеет отдельную запись с описанием. Путь к файлу не входит
в `FloatingDashboardWidgetConfig`: используется стабильный семантический ключ и sidecar-файл.
Иконки могут входить в тему через независимую область `uiIcons`; подробности — в
[Themes.md](Themes.md).

Угловые кнопки главного экрана (шестерёнка настроек, «+» добавления панели, стрелки обоев,
крестик и квадрат оконного режима) тоже в `UiIconCatalog` (`main_screen.button.*`, категория
MAIN_SCREEN) и подменяются там же / через `uiIcons`.
...
- **Долгое нажатие** по панели — режим редактирования (перетаскивание, изменение размера за угол; позиция/размер привязываются к **глобальному** `mainScreenPanelsLayoutSnapDp`, 1–50 dp, по умолчанию 1). Минимальный размер панели при resize — **2%** ширины/высоты экрана (ранее 3%).
- **Короткое нажатие** на ячейку в режиме редактирования — диалог выбора плитки.
- Вкладка диалога **«Вся панель»**: имя, строки/столбцы (**1…15**), расстояние между плитками, страница, действие по клику, индикатор отключения TBox.
...
- Позиция и размер в **пикселях** (`startX`, `startY`, `width`, `height`); минимум **25 px** при resize (ранее 50 px); **глобальный** `floatingPanelsLayoutSnapDp` — шаг привязки при drag/resize всех плавающих панелей в режиме редактирования (ручной ввод px в настройках не снапится; в темы не входит).
- **Глобальный** переключатель `floatingPanelsAllowBeyondScreen` («Перемещать за пределы экрана», по умолчанию выкл.): при включении панели в режиме редактирования не прижимаются к левому/верхнему краю и могут уходить за края экрана (`FLAG_LAYOUT_NO_LIMITS`); в темы не входит. При выключении следующие sync/drag/save снова прижимают origin к `≥ 0`.
...
- `dataKey`, `showTitle`, `titleScale`, `iconScale`, `textScale`, `shape`, цвета light/dark текста и фона плитки
...
- внешний отступ контролов: `controlPadding` (`null` = дефолт типа: музыкальные кнопки → 0, stepper/панель обдува → 6, остальные контролы → 4; 0–50 dp). Не путать с отступами ячейки. Промежутки между кнопками не входят в эту настройку.
...
- `launcherAppPackage` + режим запуска: `launcherLaunchMode` (`fullscreen` / `freeform` / `stock_window` / `virtual_display`) — для ярлыка приложения; legacy `launcherFreeformEnabled` + `launcherFreeformSide` / `launcherFreeformPercent` (20–80, шаг 10) по-прежнему читаются. Для `freeform` опциональный `launcherFreeformOverlayPage` выбирает страницу главного экрана в соседнем overlay (`null` / прочерк — не менять). Опция `launcherFreeformOverlayCrop` (`true`) рисует MainScreen в полном размере дисплея и **обрезает** его по окну overlay (viewport), а не сжимает под `w×h`. `stock_window` — штатное окно Adayo A10 (`com.adayo.launcher.LAUNCH_APP` → ActivityView). `virtual_display` — запуск через localhost ADB (`am start --display <id>`): в плитке хранится `launcherVirtualDisplayId`; общий кэш списка дисплеев (id + разрешение) — в настройках приложения (`hu_virtual_displays_json`), кнопка «Обновить список» включает ADB TCP при необходимости, опрашивает `dumpsys display`, сохраняет список и **возвращает** TCP в прежнее состояние; при тапе по ярлыку TCP включают при необходимости, запускают приложение и **оставляют** TCP включённым.
- `useMbCanVhal`, `httpRequestYaml`, поля поездки (`tripWidgetSource`, `tripMetricFieldId` для `tripMetricWidget`), `obdPidId` (для `obdMetricWidget`), `avgFuelConsumptionSource` (только `averageFuelConsumption`), `selectedDriveMode` (кнопка режима), `selectedDriveModes` (цикл режимов) и др.
...
- `speedCamShowOnMap` (только `roadMatchMapWidget`: маркеры ближайших камер/радаров SpeedCamOnline на плитке карты; по умолчанию выкл.; пишется в JSON только при `true`)
- `mapsCamShowCameras` / `mapsCamShowCurrentLimit` / `mapsCamShowAheadLimit` / `mapsCamLookaheadDistanceM` / `mapsCamRadarHoldDistanceM` / `speedCamOverageKmh` (только `osmSpeedLimitWidget`: колонки по умолчанию вкл.; дальность впереди **1000** м (300–3000, шаг **10** м); удержание лимита радара **500** м (50–3000, шаг **10** м); допуск превышения **18** км/ч (шаг **1**); в JSON пишутся только отличия от дефолтов). Старый ключ `speedCamWidget` удалён из каталога и очищается при загрузке конфига.
...
### Выбор страницы главного экрана

Виджеты **«Выбор страницы главного экрана»** (`mainScreenPageSelectorWidgetHorizontal` / `mainScreenPageSelectorWidgetVertical`, раздел «Система»): ряд или колонка из **1…N** кнопок с номером страницы (`N` = `mainScreenPageCount` в настройках). Активная страница подсвечивается control active/inactive colors (как панель обдува). Тап по номеру переключает главную; повторный тап по уже активной — no-op. В **оконном режиме** (overlay) пишет в `currentPageWindowMode` и снимает freeform session pin, как ручной свайп страниц. В каталоге вкладки **«Плитки»** типы скрыты (`WidgetsRepository.isOfferedOnMainDashboardTab`); доступны только на панелях главного экрана и плавающих панелях. Панель с виджетом по-прежнему привязана к одной странице (на другую автоматически не переносится).

### Список программ

Виджет **«Список программ»** (`appListWidget`, раздел «Система»): по нажатию открывает диалог со всеми launchable-приложениями (тот же источник, что у «Ярлык приложения»), по алфавиту, с иконкой и названием. Справа у каждой строки: **Открыть** (обычный fullscreen-запуск), **Скрыть** / **Показать**, **Удалить** (сначала подтверждение в TBox, затем системный `ACTION_DELETE` / `package:`; для чистых system-пакетов кнопка disabled). Список программ при показе системного диалога удаления **не закрывается** (закрывается только окно подтверждения). В манифесте объявлен `REQUEST_DELETE_PACKAGES` (нужен с Android 9). Скрытые пакеты хранятся в настройках (`app_list_hidden_packages`) и действуют **только** в этом списке. Переключатель **«Показать скрытые»** внизу справа — только на время открытого диалога. Иконка плитки — `dashboard.system.app_list` (заменяется в «Настроить иконки…»). Заголовок плитки настраивается как у ярлыка (showTitle / customTitle; позиция по умолчанию снизу). Диалог хостится только в `MainActivity` (`TboxApp`): Compose `Dialog` нельзя показать из оконного overlay главного экрана (`FLAG_NOT_FOCUSABLE`). Тап по плитке в оконном режиме сначала выходит в обычный fullscreen (`exitWindowModeToFullscreen`), затем открывает диалог.
...
### Ярлык приложения: виртуальный дисплей (ADB)

Режим **`virtual_display`**: запуск целевого приложения на выбранном дисплее ГУ через localhost ADB (`am start --display <id> -n package/activity`), в обход обычного `startActivity` на display 0. Список дисплеев **общий** (настройки `hu_virtual_displays_json`), по умолчанию пуст; в «Дополнительно» у ярлыка — выпадающий список и кнопка **«Обновить список»** (включает ADB TCP при необходимости → `dumpsys display` → сохраняет id и разрешение всех дисплеев, включая `0` → отключается и **восстанавливает** прежнее состояние TCP, как auto-grant `WRITE_SECURE_SETTINGS`). При тапе по ярлыку TCP включают при необходимости, выполняют `am start`, отключают сессию ADB, но **оставляют TCP включённым**. Тот же режим доступен в действии автоматизации **«Запустить приложение»** (`launchMode=virtual_display` + `virtualDisplayId`). Код: `adb/VirtualDisplayAdb.kt`, `adb/LocalhostAdbSession.kt`, `HuDisplayDumpParser`.
...
Требуется поддержка freeform на ГУ. На Adayo Android 10 (Jetour) `FEATURE_FREEFORM_WINDOW_MANAGEMENT` / `enable_freeform_support` часто **не** выставлены, но `setLaunchWindowingMode(5)` + `setLaunchBounds` всё равно работают — `hasFreeformSupport` учитывает наличие `com.adayo.launcher` и режим ГУ Android 10, а не только feature-флаги. Код: `freeform/FreeformLaunchHelper.kt`, `FreeformDisplaySpaces`, `FreeformLaunchBounds`, `FreeformCompanionSession`, `MainScreenWindowOverlayUI`, `BackgroundService` `ACTION_SHOW/HIDE_MAIN_SCREEN_WINDOW`.
...
2. **Дополнительно** — заголовок, цвета плитки (сегмент Светлая/Тёмная), блок **элементов управления** (переключатель «Цвета по умолчанию», сегмент Неактивное/Активное в паре с темой, цвет иконки/фона контрола, скругление и отступ контролов), три масштаба плитки (`titleScale` / `iconScale` / `textScale`, 0.1…2.0), скругление плитки, опции по типу виджета.
...
- **Активность** — runtime-only: после запуска приложения все триггер-виджеты неактивны, ничего не сохраняется. Переключается действием автоматизации **«Триггер автоматизации (виджет)»** (`set_automation_trigger_widget`: `stringValue` = ID, `boolValue` = активировать/деактивировать, `intValue` = `2` — переключить на противоположное состояние).
...
- **Условие автоматизации**: `trigger_widget` (`triggerId` = ID плитки, `active` = ожидаемая активность) — истинно, когда активность плитки совпадает с заданной. См. docs/AUTOMATIONS_AI_JSON_GUIDE_RU.md.
- **Вид**: заголовок занимает всю плитку — по центру, в несколько строк (с обрезкой «…»), размер подбирается под высоту плитки; цвет — активный/неактивный цвет контролов (класс Climate: активный Primary blue, неактивный — цвет текста плитки), настраивается в общем блоке элементов управления.
...
Кратко для пользователя — раздел «Права для отдельных плиток» в руководстве пользователя.
...
| Карта привязки к дорогам | `RoadMatchOverlayRepository` → Compose Canvas; без MapKit / сети / Android GPS. Follow: зум по скорости (не по GNSS), тень в центре; кнопка-защёлка «по курсу» справа сверху (не меньше 18 dp × `iconScale` плитки, на большой плитке растёт как UNIT; по умолчанию выкл., north-up); вкл. — карта плавно поворачивается по курсу тени, тень чуть ниже центра; засечки курса тени и GNSS в том же повёрнутом кадре). Состояние кнопки хранится на экземпляре плитки (`roadMatchHeadingUp`). Цвета кнопки — inactive/active control colors плитки (по умолчанию active = `#FF2180F3`). Топ-5 кандидатов матчера серым→зелёным, залипшее ребро синим. GNSS-кольцо рисуется, если попадает в кадр, но не двигает зум. Опция **«Показывать камеры и радары»** (`speedCamShowOnMap`, по умолчанию выкл.) — маркеры ближайших точек SpeedCamOnline и полупрозрачные лучи зоны подъезда (look = `DIRECTION+180°`; dirType 1/2 — трапеции, двусторонние — красный+синий; dirType 0 — диск; база из **Геопозиция → Карты, камеры и радары**). F3: «Задать» — north-up, либо тот же поворот «по курсу» (заморозка на входе); панорама, pinch-zoom и кольцо курса в этом кадре; жест курса/панорамы фиксируется на down до отпускания (без перехвата при пересечении кольца); засечка и палец на кольце — географический курс с учётом поворота; «Вставить координаты» / **Поделиться** — lat/lon + snap курса к ближайшему ребру ≤30 м (минимальный поворот, oneway); иначе курс без изменений; «Применить» пишет DR и сбрасывает matcher. Тот же seed без плитки: **Поделиться** из Яндекс.Карт / 2ГИС → TBox Monitor (режимы «Продвинутый» / «Нет фикса»). Кнопки F3 — роль UNIT + scale плитки, цвет текста плитки. Опционально F2b: подложка MapKit |
| Ограничения скорости по картам, камеры и радары | `osmSpeedLimitWidget`: три колонки (камера+дистанция / текущий знак / знак впереди+дистанция); отключённые колонки схлопываются. Текущий: OSM matched-ребро, иначе последний радар в зоне удержания. Впереди: OSM `SpeedLimitLookahead` или камера с лимитом (ближе; при Δ≤50 м — более строгое). Камера: по пути дороги при match, иначе по курсу. Дальность по умолчанию 1000 м. База камер — SpeedCamOnline (`SpeedCamPackManager`, **Геопозиция → Карты, камеры и радары**). Плитка поднимает matcher без коррекции позы (`RoadMatchDemand`) |
...
Во вкладке «Вся панель» можно задать край свайпа, толщину полоски, **толщину зоны касания** (не меньше полоски; расширяется внутрь панели; видимая полоска всегда прижата к внешнему краю), опции **«Реагировать на одиночные нажатия полоски»** и **«Реагировать на двойные нажатия полоски»** (независимы; при включённых обеих одиночный тап срабатывает после паузы ожидания второго; свайп работает всегда), цвета полоски в свёрнутом и развёрнутом состоянии (light/dark) и авто-сворачивание по одиночному или двойному тапу по плитке (включая внутренние кнопки) с задержкой 0–10 с (только если выбран край сворачивания). В **свёрнутом** состоянии фон панели/плиток не рисуется под полоской: виден только цвет полоски свёрнутого состояния; полностью прозрачный цвет → полоска не видна (зона касания и жесты остаются). Свёрнутое состояние хранится в DataStore (`panel_collapse_states`, map `panelId → bool`) **независимо от темы**. Плавающий overlay при сворачивании сжимается до полоски (окно может быть выше зоны касания, если она больше полоски). Жесты полоски обрабатывает единый детектор в `CollapsiblePanelFrame` (`PanelCollapseOverlay.kt`): тап/двойной тап/направленный свайп с одним порогом (touch slop → 24 dp), без «мёртвой зоны» между детекторами. Максимум строк/столбцов сетки плиток для панелей ГЭ и плавающих — **15** (`DASHBOARD_PANEL_MAX_GRID_DIMENSION`; вкладка «Плитки» по-прежнему до 6).
...
Плитка `gasBrakeWidget` — информационная, но с веткой в рендерере: значение газа из
`TboxDataProvider`, красный текст при `brakePedalPressedState == true`.
