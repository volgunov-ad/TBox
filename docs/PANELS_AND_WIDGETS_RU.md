...
Сериализация: `WidgetConfigCodec.kt`. Загрузка в runtime: `loadWidgetsFromConfig()`. Отступы ячейки применяются обёрткой `WidgetCellContentPadding` в сетке панели / вкладки «Плитки». Цвета, скругление и внешний отступ контролов резолвятся в `WidgetControlAppearance` и прокидываются через `LocalWidgetControlAppearance`.
...
2. **Дополнительно** — заголовок, цвета плитки (сегмент Светлая/Тёмная), блок **элементов управления** (переключатель «Цвета по умолчанию», сегмент Неактивное/Активное в паре с темой, цвет иконки/фона контрола, скругление и отступ контролов), масштаб, скругление плитки, опции по типу виджета.
