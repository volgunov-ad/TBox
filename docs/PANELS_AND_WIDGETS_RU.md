...
- `speedCamOverageKmh` / `speedCamRadiusM` / `speedCamShowOnMap` (только `speedCamWidget`: допуск превышения км/ч по умолчанию **18**, дальность **300–1000** м по умолчанию **500**, маркеры на карте привязки по умолчанию выкл.; в JSON пишутся только отличия от дефолтов / `showOnMap=true`)
...
| Карта привязки к дорогам | `RoadMatchOverlayRepository` → Compose Canvas; без MapKit / сети / Android GPS. Follow: зум по скорости (не по GNSS), тень в центре; кнопка-защёлка «по курсу» справа сверху (не меньше 18 dp × `iconScale` плитки, на большой плитке растёт как UNIT; по умолчанию выкл., north-up); вкл. — карта плавно поворачивается по курсу тени, тень чуть ниже центра; засечки курса тени и GNSS в том же повёрнутом кадре). Состояние кнопки хранится на экземпляре плитки (`roadMatchHeadingUp`). Цвета кнопки — inactive/active control colors плитки (по умолчанию active = `#FF2180F3`). Топ-5 кандидатов матчера серым→зелёным, залипшее ребро синим. GNSS-кольцо рисуется, если попадает в кадр, но не двигает зум. F3: «Задать» — north-up, либо тот же поворот «по курсу» (заморозка на входе); панорама, pinch-zoom и кольцо курса в этом кадре; жест курса/панорамы фиксируется на down до отпускания (без перехвата при пересечении кольца); засечка и палец на кольце — географический курс с учётом поворота; «Вставить координаты» / **Поделиться** — lat/lon + snap курса к ближайшему ребру ≤30 м (минимальный поворот, oneway); иначе курс без изменений; «Применить» пишет DR и сбрасывает matcher. Тот же seed без плитки: **Поделиться** из Яндекс.Карт / 2ГИС → TBox Monitor (режимы «Продвинутый» / «Нет фикса»). Кнопки F3 — роль UNIT + scale плитки, цвет текста плитки. Опционально F2b: подложка MapKit |
| Ограничение по карте (OSM) | `RoadMatchAnchorRepository` / `SpeedLimitLookahead`: круглый знак текущего `maxspeed` на matched-ребре; справа меньший знак «дальше» + дистанция, если лимит однозначен в конусе 500 м (`nextLimitHidden` скрывает). Плитка сама поднимает matcher без коррекции позы (`RoadMatchDemand`) |
| Камера SpeedCamOnline | `SpeedCamRepository` / `SpeedCamLookahead`: ближайшая точка из локальной базы igoext по курсу (конус вперёд, фильтр DIRTYPE); иконка типа + стрелка ↑ попутно / ↓ навстречу + дистанция. База — один файл с сайта SpeedCamOnline или USB (`SpeedCamPackManager`, раздел **Геопозиция → Камеры**). Превышение лимита камеры + допуск → активный цвет контролов (класс Alert / Danger). Опционально `speedCamShowOnMap` рисует точки в радиусе на плитке «Карта привязки к дорогам». Иконки типов и стрелок — `dashboard.speedcam.*` в `UiIconCatalog` (раздел Location), подмена в «Настроить иконки…» и через тему `uiIcons` |
