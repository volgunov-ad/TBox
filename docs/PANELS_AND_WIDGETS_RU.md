...
| Карта привязки к дорогам | `RoadMatchOverlayRepository` → Compose Canvas; без MapKit / сети / Android GPS. Follow: топ-5 кандидатов матчера серым→зелёным, залипшее ребро синим. F3: «Задать» панорама + pinch-zoom + кольцо курса с засечкой; «Вставить координаты» из буфера (левый нижний угол); «Применить» пишет DR и сбрасывает matcher. Кнопки F3 — роль UNIT + scale плитки, цвет текста плитки |
