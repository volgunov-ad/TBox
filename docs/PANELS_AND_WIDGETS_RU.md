...
Виджет **«Список программ»** (`appListWidget`, раздел «Система»): по нажатию открывает диалог со всеми launchable-приложениями (тот же источник, что у «Ярлык приложения»), по алфавиту, с иконкой и названием. Справа у каждой строки: **Открыть** (обычный fullscreen-запуск), **Скрыть** / **Показать**, **Удалить** (сначала подтверждение в TBox, затем системный `ACTION_DELETE`; для чистых system-пакетов кнопка disabled). Скрытые пакеты хранятся в настройках (`app_list_hidden_packages`) и действуют **только** в этом списке. Переключатель **«Показать скрытые»** внизу справа — только на время открытого диалога. Иконка плитки — `dashboard.system.app_list` (заменяется в «Настроить иконки…»). Заголовок плитки настраивается как у ярлыка (showTitle / customTitle; позиция по умолчанию снизу).
