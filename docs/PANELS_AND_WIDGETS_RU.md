...
- `mapsCamShowCameras` / `mapsCamShowCurrentLimit` / `mapsCamShowAheadLimit` / `mapsCamLookaheadDistanceM` / `mapsCamRadarHoldDistanceM` / `speedCamOverageKmh` (только `osmSpeedLimitWidget`: колонки по умолчанию вкл.; дальность впереди **1000** м (300–3000); удержание лимита радара **500** м; допуск превышения **18** км/ч; в JSON пишутся только отличия от дефолтов). Старый ключ `speedCamWidget` удалён из каталога и очищается при загрузке конфига.
