...
| Карта привязки к дорогам | `RoadMatchOverlayRepository` → Compose Canvas; без MapKit / сети / Android GPS. Follow: зум по скорости (не по GNSS), тень в центре; кнопка-защёлка «по курсу» справа сверху (по умолчанию выкл., north-up; вкл. — карта плавно поворачивается по курсу тени, тень чуть ниже центра; засечки курса тени и GNSS в том же повёрнутом кадре). Состояние кнопки хранится на экземпляре плитки (`roadMatchHeadingUp`). Цвета кнопки — inactive/active control colors плитки (по умолчанию active = `#FF2180F3`). Топ-5 кандидатов матчера серым→зелёным, залипшее ребро синим. GNSS-кольцо рисуется, если попадает в кадр, но не двигает зум. F3: «Задать» north-up, панорама + pinch-zoom + кольцо курса с засечкой; «Вставить координаты» из буфера (левый нижний угол); «Применить» пишет DR и сбрасывает matcher. Кнопки F3 — роль UNIT + scale плитки, цвет текста плитки |
