...
`--kinematic` снимает из дельты курса полевой `bearingDeltaDeg` и двигает позу
на длину шага вдоль восстановленного DR-курса, затем снова применяет matcher.
Так можно проверить контрфактический catch-up (например, не крутить `*_link`
на прямой, лог `142148`) без повторной поездки. Обычный режим и baseline
без этого флага не меняются.
