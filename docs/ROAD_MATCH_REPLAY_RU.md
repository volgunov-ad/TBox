...
`--kinematic` двигает позу по `integ.dDistM` и крутит курс по
`dYawDebDeg * yawScale` (без полевого match-yaw и без закрученного mock-пути),
затем снова применяет matcher. Так проверяется контрфактический catch-up
(не крутить `*_link` на прямой, лог `142148`) без повторной поездки.
Обычный режим и baseline без этого флага не меняются.
