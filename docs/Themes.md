...
- **задержка 3 с** после последнего изменения режима: при быстром переключении ECO → SPORT → … тема применяется только когда режим стабилизировался (не на каждом промежуточном значении);
