...
3. Выполняются два этапа:
   ...
   - **Активация** (быстро): импорт `theme.json`, установка URI обоев и фонов плиток; иконки **не копируются** — виджеты читают их из кэша активной темы (см. ниже).
...
4. Фоны плиток копируются в `files/tile_backgrounds/`.
...
| Фоны плиток | **нет** (только файлы на диске) | да | да |
...
| Класс | Назначение |
|-------|------------|
| `LauncherAppIconPaths` | Общая и кэш-тема: резолв пути, список пакетов |
...
- `files/tile_backgrounds/` — пользовательские фоны плиток;
