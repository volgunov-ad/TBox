...
На диске иконка хранится как `launcher_app_icons/{packageName}` **без расширения** (например, `com.example.music`). В архиве темы файл кладётся как `assets/icons/{packageName}` — тоже без `.png`.

Если экспортируются **только** иконки (без панелей), в архив попадают **все** пользовательские иконки из папки (кроме служебных файлов вроде `.txt`).
...
3. Выполняются два этапа:
   ...
   - **Активация** (быстро): импорт `theme.json`, установка обоев, копирование иконок и фонов плиток в рабочие каталоги.
...
3. Иконки копируются в `files/launcher_app_icons/`.
4. Фоны плиток — в `files/tile_backgrounds/`.
...
| Иконки приложений | **нет** (только файлы на диске) | да (опционально) | да |
...
- `files/launcher_app_icons/` — иконки виджетов «Ярлык приложения»;
