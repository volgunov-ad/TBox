...
- `currentPageWindowMode` — страница в оконном режиме (freeform companion); отсутствует, пока пользователь ни разу не листал страницы в оконном режиме и ни один freeform-ярлык не выбирал страницу overlay. Ярлык с явной страницей обновляет это поле перед запуском; прочерк не меняет его.
---
