...
2. Если для корректного JSON не хватает динамического значения, задай короткий уточняющий
   ...
   - координаты геозоны;
   - модель ГУ A9/mbCAN или A10/VHAL только если без семантики нельзя (для бинарных
     предпочитай `"on"`/`"off"`, для стёкол — `"close"`/`"open"`/`"vent"`);
   - `keyCode` для триггера `hard_key`, если пользователь не назвал кнопку из таблицы
     ...
     руля зависит от модификации авто; на A10 `hard_key` не работает — предложи виджет-триггер;
...
### Нажатие виджета-триггера

```json
{"type":"widget_pressed","id":"1","triggerId":"myButton1"}
```
...
```json
{"type":"interval","id":"1","intervalMillis":60000}
```

- `intervalMillis`: `2000..86400000` мс и обязательно кратно 1000 (целое число секунд);
- отсчёт начинается, когда фоновая служба полностью запущена;
- первый запуск — через полный интервал, не сразу;
- расписание привязано к монотонному времени запуска службы и не зависит от системных часов;
- после перезапуска службы отсчёт начинается заново;
- пропущенные границы схлопываются в один запуск без выполнения пачкой;
- включение или правка правила пропускает прошлые границы и ждёт следующую границу общей сетки;
- если текущий запуск отклонён условиями, cooldown или `runMode`, новая попытка будет только
  на следующей периодической границе.

Для «сразу после запуска, затем периодически» добавь в правило два OR-триггера:
`background_service_started` и `interval`.
...
### Состояние виджета-триггера

```json
{"type":"trigger_widget","triggerId":"прогрев","active":true}
```

- `triggerId` — ID плитки «Триггер автоматизации» (до 32 символов, тот же ID, что у действия
  `set_automation_trigger_widget` и триггера `widget_pressed`);
- `active` — ожидаемая активность: `true` — плитка активна, `false` — неактивна. Условие истинно,
...
- для **бинарных** `set` предпочитай семантику `"on"` / `"off"` (portable A9+A10). Legacy
  int по шкале A9/mbCAN (`1` выкл / `2` вкл у большинства; рециркуляция — наоборот) по-прежнему
  принимается; на A10 приложение само перекодирует через `VhalBinaryToggleCodec`;
- для **стёкол** предпочитай `"close"` / `"open"` / `"vent"` (или `"comfort_open"` = A9 80%).
  Legacy A9 `0/20/80/100` и A10 `1/2/3` при выполнении ремапятся под текущий backend;
- прочие небинарные значения по-прежнему в шкале A9/mbCAN с преобразованиями A10 ниже;
...
Для строк ниже разрешены `operation: "set"` и `"toggle"`. Для `set` предпочитай
`"value": "on"` или `"off"`. Допустимы и legacy-числа по колонке A9; на A10 они
перекодируются автоматически. Колонка A10 ниже — фактический raw на шине после remap,
не то, что нужно писать в JSON.
...
| Управление кондиционером | 36 | A9+A10 | 1 выкл, 2 вкл | 1 выкл, 2 вкл |
...
Для окон обязательно спроси backend ГУ только если пишешь legacy-числа. Предпочитай
семантику `"close"` / `"open"` / `"vent"` / `"comfort_open"` — она работает на A9 и A10.
Не переноси значение A9 в A10 или наоборот без remap.
...
### Раз в 30 секунд показывать Toast
...
```json
{
  ...
  "automations": [
    {
      "id": "f3500779-6d9e-48f5-b467-cf039570af60",
      "name": "Периодическое уведомление",
      "description": "Показывает сообщение раз в 30 секунд от запуска службы",
      "enabled": false,
      "triggers": [
        {
          "type": "interval",
...
### В будни в 07:30 включить обогрев руля, если температура ниже 5 °C
  ...
  "automations": [
    {
      ...
      "name": "Утренний обогрев руля",
      "description": "Включает обогрев руля в холодное утро (portable on/off для A9 и A10)",
      "enabled": false,
      "triggers": [
        {
          "type": "time",
...
Legacy `"value": 2` (A9 on) тоже валиден и на A10 перекодируется автоматически.
Для A10/VHAL больше не нужно подставлять raw `1` вручную.
