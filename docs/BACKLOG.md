...
| # | Задача | Источник | Область кода | Сложн. |
|---|--------|----------|--------------|--------|
| F-01 | **Кнопка «Обновить данные по заправке»** — в меню «Заправки» на **каждой строке** (повторная подтяжка цены АЗС / пересчёт, когда появился интернет). | «…кнопку - обновить данные по заправке в меню Заправки, когда интернет появился» | `fuel/`, вкладка заправок UI, сетевой слой цен | S *(реализовано: per-row)* |
