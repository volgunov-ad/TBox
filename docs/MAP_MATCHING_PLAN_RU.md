...
5. Пока set-mode открыт, живая тень на экране не дёргает маркер (показывается черновик). GNSS-кольцо можно оставить для ориентира.
...
- [ ] F3: «Задать» → панорама + pinch-zoom (центр = новая тень) + кольцо курса → «Применить» двигает DR и сбрасывает matcher / «Отмена» откатывает; long-press в set-mode не открывает edit.
...
9. **PR6b (F3):** ручной seed на Canvas: «Задать» / панорама+кольцо / «Применить»·«Отмена».  
