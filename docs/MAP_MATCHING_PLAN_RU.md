...
| **Кнопка** «Карты дорог…» | Открывает **отдельное окно** (hub dialog, как калибровки гиро/руля). В **Геопозиция → Карты, камеры и радары**; доступна всегда (скачивание не зависит от mock). |
...
- [x] Собрать и синхронизировать все 96 пакетов: remote `/maps/catalog.json` —
      96/96 с `yandex-disk:` URL и ненулевым `bytes`, `graphVersion` 4;
      `build_report.json` — `ok=96`, `failed=0`; на шаре 96 `*-v4.tboxroads.zip`.
      Пакеты содержат `oneway` и `maxspeed` (выборочно Москва/Минск).
- [x] Проверить размеры/качество нескольких полных областей на HU (полевые
      прогоны + geo-debug журналы; дальше — подбор тюнинга матчера).

**Результат:** graphVersion 4, fallback-список offline; полный remote-каталог
96 регионов на публичной папке Яндекс.Диска `/maps/` без обновления APK.
...
- [x] Режим привязки **Обычный** / **Рельсы** / **Повороты** (default Обычный): сегмент на
      ...
      **Повороты** (эксперимент): те же параметры, что Обычный, но catch-up
      курса к выбранному (синему) ребру 26°/тик (и 18° на throttled-тиках).
      За 35 м до узла, где сходится **более 2 линий** (3+ ребра — любой
...
- [x] Экспериментальное меню тюнинга matcher: общий цикл/триггеры, поиск и
      ...
      хранит в DataStore JSON только отклонения; group/all reset удаляет overrides
      и тем самым возвращает production defaults текущей версии. Диапазоны
...
- [x] Полевой replay на HU (журналы с GNSS + искусственное скрытие): журналы
      собраны и проанализированы; текущий фокус — подбор параметров тюнинга
      привязки (`RoadMatchTuning`), а не сбор новых логов.
- [x] Массовая пересборка v4 bundles с shared nodes **и oneway** (все 96 на
      Яндекс.Диске; выборочная проверка тайлов — поля присутствуют).
...
Отдельный горизонт по умолчанию **1000 м** (`SpeedLimitLookahead`, настраивается в плитке), не короткий match-коридор
...
2. После нажатия кнопка сменяется на **«Применить»** и **«Отмена»**. Плитка входит в set-mode:
   ...
   - жест **фиксируется на down**: старт на кольце → до отпускания только курс (даже если палец ушёл с кольца); старт вне кольца → только панорама/pinch (даже если палец пересёк кольцо);
   - кнопка **«Вставить координаты»** (левый нижний угол, тот же стиль UNIT): lat/lon из буфера (Яндекс/Google/2GIS/DMS/`N E S W`/с.ш. в.д.); курс выравнивается по ближайшему ребру ≤30 м (минимальный поворот, с учётом oneway), иначе не меняется. Если строка не распозналась — надпись краснеет на 2 с;
   - тот же seed без плитки: Android **Поделиться** (`ACTION_SEND` `text/plain` / `geo:`) → `GeoShareReceiverActivity` → `GeoCoordinateParse` → `RoadMatchManualSeedRepository` (режимы **Продвинутый** / **Нет фикса**); тот же snap курса по ребру ≤30 м;
...
- Для совместимости, если `file` отсутствует, можно использовать basename из
  `url`, но **не** обращаться в сеть. Строки без `file`/`url` пропускаются
  (неопубликованные регионы remote-каталога).
- `file` — только имя или относительный путь внутри выбранной папки; запретить
  абсолютные пути, `..`, URI-схемы и выход из дерева.
- Опубликованный `maps/catalog.json` с Яндекс.Диска **не содержит** `sha256`,
  только `bytes`. Установка с флешки сверяет размер при копировании, если
  `bytes` > 0; `sha256` проверяется только когда в каталоге есть 64 hex.
  ZIP и заголовки тайлов всегда валидируются установщиком.
...
- [x] Выбор JSON-каталога с USB через SAF без `MANAGE_EXTERNAL_STORAGE`.
- [x] Список регионов с выбором по одному и «выбрать все».
- [x] Корректный import v4 bundle из той же папки, проверка size/hash/header.
- [x] Атомарное обновление: при ошибке/извлечении флешки старая карта остаётся.
- [x] Проверка места, прогресс и отмена; temp-файлы очищаются.
- [x] Ошибки понятны: плохой JSON, отсутствует файл, hash/version/id mismatch,
      USB извлечён, нет места.
- [x] После импорта matcher и F2a видят карту без перезапуска приложения.
- [x] Unit-тесты каталога/валидации/path traversal/rollback; ручной тест на HU
...
Этапы **A–C закрыты** (все пункты §6 отмечены). Критерии ниже — приёмка MVP:

- [x] Default off — бит-в-бит текущий DR.
- [x] Тумблер доступен в Always-on (enhance) и «Нет фикса».
- [x] Окно скачивания: страны из ТЗ, области, download/delete/progress.
- [x] При вкл. + покрытие: поперечная ошибка уменьшается без скачка одометра/длины пути.
- [x] На синтетическом «повороте» возможен switch ребра.
- [x] Geo-debug показывает активность match.
- [x] Документация USER_GUIDE + этот план актуален.
