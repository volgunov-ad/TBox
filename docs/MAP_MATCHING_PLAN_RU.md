...
2. После нажатия кнопка сменяется на **«Применить»** и **«Отмена»**. Плитка входит в set-mode:
   ...
   - **тень прибита к центру** плитки, панорама двигает карту (дороги) под ней — центр кадра = черновая lat/lon;
...
- [ ] F3: «Задать» → панорама (центр = новая тень) + кольцо курса → «Применить» двигает DR / «Отмена» откатывает; long-press в set-mode не открывает edit.
