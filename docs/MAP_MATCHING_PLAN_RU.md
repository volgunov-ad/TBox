...
- [x] Hint поворотника на развилке: `UniversalCanRepository.turnSignalsLatchedSide`
  ...
  На **изогнутой односторонней дуге** (кольцо: oneway, ≤120 м, изгиб ≥35°) полный
  hint не включается (look-ahead / inhibit / past-end). Остаётся слабый сдвиг
  score (`TURN_SIGNAL_ARC_WEIGHT` 0,35): курс и same-node наследник главнее,
...
- [x] При softCorrect к matched-ребру курс ловится быстрее (до 14°/тик).
      ...
      На том же ребре `*_link` курс к азимуту рампы **не** ловят (ни catch-up,
      ни fade-blend): иначе прямая по motorway + ранний `trunk_link` крутит
