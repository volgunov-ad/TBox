...
- [x] Пилотные городские пакеты OSM (Overpass): Москва, Нижний, Крым/Симферополь, ДНР, ЛНР, Минск, Алматы, Ереван, Баку, Ташкент — в APK `asset://`.
- [x] Tool: `--fetch-overpass` / `--overpass-json`; батч `tools/build_road_map_packs.py`.
- [x] Хостинг крупных пакетов: [ROAD_MAPS_HOSTING_RU.md](ROAD_MAPS_HOSTING_RU.md) (GitHub Releases + `https://` в каталоге).
- [ ] Полные субъекты / «Беларусь (вся)» — через release, когда размер приемлем на ГУ.

**Результат:** скачиваемые OSM-пакеты graphVersion 2; update со старых демо.
