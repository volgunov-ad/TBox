...
- [x] Поперечный поводок: при уходе (датчики против ребра, или residual растёт
      **и** xt≳4 м) не тянуть lat/lon к полотну; при `xt`≳12 м и нескольких метрах
      ухода — `leash_break` (без corridor). На развилке 3+ исходящих, либо 3+
      кластера курса **во время поворота**, — виртуальная точка по приборам;
      после успокоения манёвра, если она уехала ≥15 м (или ≥8 м и курс ≥30°) —
      она становится основной. Только курс / недокрут на ребре (xt мал) не
      промоутит (кейс `124442`).
