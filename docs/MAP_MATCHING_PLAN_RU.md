...
- [x] Поперечный поводок: при уходе (курс растёт / датчики против ребра) не тянуть
      lat/lon к полотну; при `xt`≳12 м и нескольких метрах ухода — `leash_break`
      (без corridor). На развилке 3+ направлений — виртуальная точка по приборам;
      после успокоения манёвра, если она уехала ≥15 м (или ≥8 м и курс ≥30°) —
      она становится основной. Только курс без сдвига места не промоутит
      (недокрут гиро, кейс `124442`).
