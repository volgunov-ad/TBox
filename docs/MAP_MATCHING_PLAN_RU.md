...
- [x] Режим привязки **Обычный** / **Рельсы** / **Повороты** (default Обычный): сегмент на
      ...
      **Повороты** (эксперимент): те же параметры, что Обычный, но catch-up
      курса к выбранному (синему) ребру 28°/тик (и 20° на throttled-тиках).
      За 30 м до узла, где сходится **более 3 линий** (4+ ребра), поза
      полностью отвязывается (`skippedReason=free_turns_junction`); привязка
      снова после 10 м за этим узлом. Т-образные (3 линии) не отпускают.
