...
| **Android 9** — ReverseGearSwitch | `readReverseGearSwitch()` / `MBCanVehicleBcmStatus.getReverseGearSwitch()` (type **21**) | **0** false / **1** true (`decodeReverseGearSwitch`) | — | **Push:** `onVehicleBcmStatusChange` + pull. StateFlow `reverseGearSwitchState`. Mock/DR: OR с PRND `R` (`VehicleGearDomain.isReverseEngaged`); интерес `mock-location-dr-gear` при enhance-режимах |
