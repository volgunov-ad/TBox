...
Виджет `mirrorFoldWidget`: фактическое положение зеркал недоступно, поэтому приложение запоминает последнюю **успешно отправленную** команду (`MirrorFoldLastCommandStore`, SharedPreferences, переживает перезагрузку).

- **Одиночное нажатие** — отправляет противоположную команду относительно последней (toggle). По умолчанию (ещё не было отправок) последняя считается **unfold (2)**, значит первый одиночный тап шлёт **fold (1)**.
- **Двойное нажатие** — всегда **fold (1)** и обновляет запомненную команду на fold.
