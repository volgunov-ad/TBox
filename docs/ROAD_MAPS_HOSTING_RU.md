...
на USB (SAF). Рядом с JSON должны лежать ZIP-пакеты из поля `file` (относительный путь).
...
`bytes` и `sha256` обязательны. Пути с `..`, абсолютные и URI отклоняются. Если
провайдер USB не отдаёт sibling-файлы, UI просит выбрать папку (`OpenDocumentTree`).
Пакеты копируются во внутренний `road_maps/` с проверкой hash и атомарной заменой
каталога `{id}.tboxroads.d` — как при скачивании с Яндекс.Диска.
