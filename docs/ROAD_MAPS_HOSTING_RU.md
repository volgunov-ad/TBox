...
Если провайдер USB не отдаёт sibling-файлы, UI просит выбрать папку
(`OpenDocumentTree`). Пакеты копируются во внутренний `road_maps/` с атомарной
заменой каталога `{id}.tboxroads.d`.
